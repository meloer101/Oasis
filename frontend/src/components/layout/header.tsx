...
export default function Header() {
  ...
  return (
    <header
      className={`sticky top-0 z-30 ${HEADER_H} border-b border-[var(--card-border)] bg-[var(--topnav-bg)] backdrop-blur-md flex items-center gap-2 sm:gap-3 px-3 sm:px-4`}
    >
      <button
        type="button"
      ...
      {/* Desktop search */}
      <form onSubmit={onSearchSubmit} className="hidden sm:flex flex-1 max-w-xl mx-2">
      ...
      <div className="relative shrink-0" ref={menuRef}>
        <button
          ...
          aria-haspopup="true"
        >
          <div className="w-9 h-9 rounded-full bg-emerald-800 flex items-center justify-center text-sm font-semibold text-emerald-200">
            {initials}
          </div>
        </button>
        {menuOpen && (
